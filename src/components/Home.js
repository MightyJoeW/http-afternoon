...
class Home extends Component{
    ...
    componentWillMount() {
        axios.get('/api/featured').then(results => { //results is a callback that could have been named anything
            this.setState ({
                featured: results.data,
                index: (~~(Math.random() * results.data.length) + 0), // randomly select a post to feature on the homepage
                posts: results.data
            })
        }).catch(console.log);
    }
